...
void TimeTask::updateKodiPlayback(bool value) {
  kodiPlayback = value;
}

void TimeTask::updateKodiPlaybackTime(const char* value) {
  kodiPlaybackTime = value;
}
 ...
 */
  ...
  // Activate Kodi Playbacktime by Second 30
  if (s >= 30  && s <= 45 &&_state == Run) {
    if (kodiPlayback) {
      // Serial.println(kodiPlaybackTime);
      sscanf(kodiPlaybackTime, "%d:%d:%d", &h, &m, &s);
    }
  }
