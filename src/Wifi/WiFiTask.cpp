...
const char* broker = BROKER_IP;           // Stores the Broker IP from Secrets.h
const char* username = MQTT_USERNAME;     // Stores the Username from Secrets.h
const char* password = MQTT_PASSWORD;     // Stores the Password from Secrets.h
const char* topic1  = "CLOCK/Nixie_Display_on";
const char* topic2  = "CLOCK/Nixie_Display_control_on";
const char* topic3  = "CLOCK/Nixie_Display_brightness";
const char* topic4  = "CLOCK/Nixie_Display_control_brightness";
const char* topic5  = "CLOCK/Nixie_Display_kodi_hc";
const char* topic6  = "kodi_hc/status/progress";
const char* json;
...
void WiFiTask::connectMQTT() {
  // You can provide a username and password for authentication

  client.begin(broker, net);
  client.connect("Nixie-Clock", username, password);
  client.onMessage(messageReceived);
  }

// Update Display Status per MQTT 
void WiFiTask::displaystatusMQTT() {
   if (!client.connected()) {
    connectMQTT();
  }
   client.loop();
   if (_hv->isOn()) {
    client.publish(topic1, "true");
    } else {
    client.publish(topic1, "false");
    }
    client.publish(topic3, String(_settings->flashBrightness));
  }

void WiFiTask::MQTTmessage() {
  ...
  if(recTopic == topic4) {
    // set Brightnesslevel 
    _settings->flashBrightness = recPayload.toInt();
    _settings->rwSettings(18, 1);
    _fade->setBrightness();
    // Turn Display on when brigtness higher then 0
    if(recPayload.toInt() > 0) {
      _hv->switchOn();
    }
  }
  if(recTopic == topic5) {
    if(recPayload == "true") {
      _timetask->updateKodiPlayback(true);
    }
    if(recPayload == "false") {
      _timetask->updateKodiPlayback(false);
    }
  }
  if(recTopic == topic6) {
    json = recPayload.c_str();
    DynamicJsonDocument doc(capacity);
    DeserializationError error = deserializeJson(doc, json);
    kodi_time_value = doc["kodi_time"];
    if (kodi_time_value != nullptr) { 
      // Time Kodi live Position
      // Serial.println(kodi_time_value);
      _timetask->updateKodiPlaybackTime(kodi_time_value);
    }
  }   
}

void WiFiTask::messageReceived(String &topic, String &payload) {
  // MQTT Traffic control
      ...
      if (client.available()) {
        ...
        if (c == '\n') {
          if (currentLine.length() == 0) {
            ...
            CP("<span class='btn-text' onclick='lux.submit()'>Set Low Lux Level</span>");
            CP("</a></form>");
            ...
            CP("<h1>&copy; 2021 : Ratti3 Technologies Corp</h1>");
            CP("</div></body></html>");
            CPL();
            break;
          } else {
